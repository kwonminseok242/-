...
def generate_future_state_windows(df, sequences, window_size=50, stride=1, future_offset=18):
    """
    과거 시퀀스를 기반으로 미래 센서값을 예측하는 학습 데이터를 생성합니다.
    df : 원본 DataFrame (timestamp, state, 센서값)
    sequences : 연속 시계열 구간 리스트
    """
    X, y = [], []

    for start, end, _, _, _ in sequences:
        sub_df = df.iloc[start:end + 1].reset_index(drop=True)
        max_start = len(sub_df) - window_size - future_offset
        for i in range(0, max_start + 1, stride):
            window = sub_df.iloc[i:i + window_size]
            future = sub_df.iloc[i + window_size + future_offset - 1]

            features = window.drop(columns=['timestamp', 'state']).values
            future_sensor_values = future.drop(labels=['timestamp', 'state']).values  # ✅ 센서값만

            X.append(features)
            y.append(future_sensor_values)

    X = np.array(X, dtype=np.float32)
    y = np.array(y, dtype=np.float32)  # ✅ float32 for 회귀
    return X, y
...
def save_windows(X, y):
    ...
    np.save(os.path.join(save_dir, "X_windows_predict.npy"), X)
    np.save(os.path.join(save_dir, "y_states_pridict.npy"), y)
    print(f"[✅] 저장 완료: {save_dir}")

if __name__ == "__main__":
    # CSV 파일 경로
    current_dir = os.path.dirname(os.path.abspath(__file__))
    csv_path = os.path.join(current_dir, "..", "data", "T_f_t_d.csv")
